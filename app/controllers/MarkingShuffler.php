...
class MarkingShuffler {
    ...
    // the marking task is then assigned to a tutor or to the lecturer
    // (depending on the settings of Assignment::late_policy)
    ...
    // returns the users nicely grouped.
    // The key of the array is either the group_id or
    // the negative of the user_id (if there is no group).
    private static function collectGroups($assignment, $user_ids) {
        ...
        $combinedUsers = array_unique(array_merge($user_ids, array_keys($groupList)));

        foreach($combinedUsers as $id) {
        ...
        foreach($groupDefinition as $gid => $users) {
            $groupExists = false;

            foreach($users as $user) {
                $groupExists |= in_array($user, $user_ids);
            }

            if(!$groupExists) {
                unset($groupDefinition[$gid]);
            }
        }
    ...
    private static function improveGroupShuffleRandomness($answerDefinition, $shuffledData, $logger)
    {
        // collect the request positions we have.
        $allPositions = array();
        foreach($shuffledData as $originalUser => $answerIdxList) {
            foreach($answerIdxList as $answerIdx => $answerId) {
                $allPositions[] = array($originalUser, $answerIdx);
            }
        }

        $userList = array_unique(array_keys($shuffledData));
        $groupList = array();
        foreach($userList as $user) {
            foreach($answerDefinition as $idx => $data){
    ...
    private static function createGroupReviewTasks($questionOrAssignment, $shuffledData, $role, $answerDefinition) {
        $reviewTasks = array();
    ...
    private static function createReviewTasks($questionOrAssignment, $shuffledData, $role, $groupList = array(), $userAnswers=NULL) {

        // get all the answers
        if ($userAnswers == NULL) {
            $userAnswers = array();
            foreach ($answerList = $questionOrAssignment->submittedAnswers()->get(array('id', 'user_id')) as $answer) {
        ...
        foreach($shuffledData as $user_id => $tasks) {
            foreach($tasks as $task) {
                foreach($userAnswers[$task] as $answer_id) {
                    ...
                    if(isset($groupList[$user_id])) {                                       // If User is in group
                        $group = array();                                                   // Construct group
                        foreach(array_keys($groupList, $groupList[$user_id]) as $member) {  // Get and loop over all users with same group id
                            if ($member != $user_id) {                                      // For users not primary user
                                $group[] = $member;                                         // Add user to group
                            }
                        }
                        if (count($group) > 0) {                                            // If more than primary user in group
                            $newTask['other_group_members'] = $group;                       // Outline other users in task
                        }
                    }
        ...
        if ($assignment->usesPeerReview()) {
            ...
            if ($assignment->onlyOneAnswerPerGroup()){
                ...
                foreach($answerResult as $answer) {
                    ...
                    foreach($groupDefinition as $group => $users) {
                    ...
                    if (!$answerHandled) {
                        $answer['group_id'] = 0;
                        $answer['users'] = array($userId);
                    }
                }

                $answerDefinition = array();
                foreach($answerResult as $answer) {
                    $userId = $answer['user_id'];
                    $answerHandled = false;
                ...
                $shuffledData = self::createGroupShuffleDataForPeerReview($assignment, $answerDefinition, $logger);

                $peerReviewCount = self::createGroupReviewTasks($questionOrAssignment,
                    $shuffledData,
                    ReviewReviewerRole::student,
                    $answerDefinition);
            } else {
                $shuffledData = self::createShuffleDataForPeerReview($assignment, $user_ids, $logger);
                $peerReviewCount = self::createReviewTasks($questionOrAssignment,
                    $shuffledData,
                    ReviewReviewerRole::student);
            }
    ...
    public static function shuffleAssignment(Assignment $assignment) {
        ...
        if ($assignment->keepAssignmentTogetherWhenShuffling()) {
            ...
            foreach($assignment->questions_ordered as $question) {
                ...
                $logger = function($level, $text) use ($assignment, $question) {
                    $assignment->logEvent($level,
                        "Creating peer review tasks (question ". $question->presenter()->full_position ."): ". $text);
                };
        ...
        if ($assignment->keepAssignmentTogetherWhenShuffling() && $assignment->tutorsAreIdenticalforAllQuestions()) {
            ...
            foreach($assignment->questions_ordered as $question) {
                ...
                $logger = function($level, $text) use ($assignment, $question) {
                    $assignment->logEvent($level,
                        "Creating tutor review tasks (question ". $question->presenter()->full_position ."): ". $text);
                };
        ...
        foreach($assignment->questions_ordered as $question) {
            ...
            $logger = function($level, $text) use ($assignment, $question) {
                $assignment->logEvent($level,
                    "Creating lecturer review tasks (question ". $question->presenter()->full_position ."): ". $text);
            };
